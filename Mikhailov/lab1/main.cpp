...
private:
    ...
    void insertDivision(size_t x, size_t y, size_t size_)
    {
        ...
        currDivCount++;
        std::cout<<"\t\t~~~~~Insert~~~~~\n";
        std::cout<<"\tcurrDivCount: "<<currDivCount<<"\n";
        std::cout<<"\tsquareDivArr:\n";
        for( unsigned int i = 0; i < squareDivArr.size(); i++ )
        {
            for( unsigned int j = 0; j < squareDivArr[i].size(); j++ )
            {
                std::cout << squareDivArr[i][j] << " ";
            }
            std::cout << std::endl;
        }
    }
    ...
    void updateOptimalDivision(size_t x, size_t y,int deep = 0)
    {
        ...
        {
            std::cout<<"currDivCount >= optimalDivCount, end\n";
            return;
        }
        for (auto n = size / 2; n > 0; --n)
        {
            std::cout<<"Step: "<<size / 2 - n<<";\n\tn == "<<n<<";\n";
