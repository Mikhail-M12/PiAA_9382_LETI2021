...
std::vector<int> countPrefixFunction(std::string str) 
{
	...
	std::vector<int> prefixes(stringLen);   // вектор из значений максимальных префикс-суффиксов
    ...
    {
        int borderPrefixLen = prefixes[i - 1];  // это значение граничное (оно может увеличиться лишь на 1)
		int prefixLen = borderPrefixLen;

#ifdef ADDINFO
    std::cout << "\nРассматриваем подстроку = \"" << str.substr(0, i + 1) << "\"." << std::endl;
    std::cout << "Длина \"граничного\" префикса предыдущей подстроки: " << str.substr(0, prefixLen) << std::endl;
#endif        

		while (prefixLen > 0 && str[prefixLen] != str[i])   // если последние символы не совпали - уменьшаем длину
        {
			prefixLen = prefixes[prefixLen - 1];            // полагая ее равной p(k-1)
            
#ifdef ADDINFO
    std::cout << "Крайние символы не совпали -> переходим к длине префикса = " << prefixLen << "." << std::endl;
#endif
		}		

		if (str[prefixLen] == str[i])  // если последний добавленный символ равен последнему символу в только что увеличенном префиксе
        {
			prefixLen += 1;            // увеличим длину префикса и переходим к следующему индексу

#ifdef ADDINFO
    std::cout << "Крайние символы совпали -> увеличиваем длину префикса на 1." << std::endl;
#endif
        }
		else if (prefixLen != 0)                    // если же мы дошли до длины 0, но так и не нашли совпадения
        {
			prefixLen = prefixes[prefixLen - 1];    // то останавливаем перебор, полагаем значение равным 0, и переходим к следующему индексу

#ifdef ADDINFO
    std::cout << "Мы дошли до минимальной длины, но так и не полуили совпаение." << std::endl;
#endif    
		}
...
#ifdef ADDINFO
    std::cout << "На этом шаге в итоге получен префикс длины = " << prefixLen << "." << std::endl;
#endif
	}
...
std::vector<int> findPatternWithKMP(std::string text, std::string pattern)
{
	...
	for (int i = 0; i < n; i++) 
    {
		...
		while (match > 0 && pattern[match] != text[i])  // обновление совпадающей части
        {
#ifdef ADDINFO
			std::string pre = "_";
		...
		if (pattern[match] == text[i])  // если символы на текущей позиции в строках совпадают
        {
#ifdef ADDINFO
	std::cout << "Символы совпадают. Увеличение длины совпадающей части\n\n";
        ...
        {
#ifdef ADDINFO
	std::cout << "Найдено совпадение\nИндекс начала искомой строки в тексте: " << i - match + 1 << std::endl;
#endif

			indexes.push_back(i - match + 1); // то добавим это соввпадение в список индексов
		}
