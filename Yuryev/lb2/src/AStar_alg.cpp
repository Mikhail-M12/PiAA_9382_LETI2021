            ...
            for(int k = 0; k < currentPoint->m_edgesFromPoint.size(); k++)
            {
                ...
                if(currentEdge->m_pointTo->m_isVisited == false)
                {
                    ...
                    if(currentEdge->m_pointTo->m_distanceFromStart == -1)
                    {
                        ...
                        currentEdge->m_pointTo->m_priority = currentEdge->m_pointTo->m_distanceFromStart + heuristicFunc(currentEdge->m_pointTo->m_nameOfPoint, finishPoint->m_nameOfPoint);
                    ...
                    else if( (currentPoint->m_distanceFromStart + currentEdge->m_length + heuristicFunc(currentEdge->m_pointTo->m_nameOfPoint, finishPoint->m_nameOfPoint)) <  currentEdge->m_pointTo->m_priority) // TO DO: возможно для А* нужно будет <=
                    {
                        currentEdge->m_pointTo->m_distanceFromStart = currentPoint->m_distanceFromStart + currentEdge->m_length;
                        currentEdge->m_pointTo->m_priority = currentEdge->m_pointTo->m_distanceFromStart + heuristicFunc(currentEdge->m_pointTo->m_nameOfPoint, finishPoint->m_nameOfPoint);
                        currentEdge->m_pointTo->m_cameFrom = currentEdge;
